...
vec4 castRay(vec4 p0, vec4 E, Object *lastHitObject, int depth){
  ...
  Object::IntersectionValues valeurIntersect;
  Object::IntersectionValues closestValues;
  bool hitObject = false;
  Object* closestObject = lastHitObject;
  double distanceMin = std::numeric_limits< double >::infinity();


  //trouve l'intersection avec l'objet le plus proche et retourne sa couleur

  for (int i = 0; i < sceneObjects.size(); i++)
  {
    valeurIntersect = sceneObjects[i]->intersect(p0, E);
    if(distanceMin > valeurIntersect.t && valeurIntersect.ID_ != -1){
      distanceMin = valeurIntersect.t;
      closestValues = valeurIntersect;
  ...
  if(hitObject){
    vec4 lightDir = normalize(lightPosition - closestValues.P);
    double distLight = sqrt(dot(lightDir, lightDir));
    
    for(int i = 0;  i<sceneObjects.size(); i++){
      if(closestValues.t != std::numeric_limits< double >::infinity() && (closestObject != sceneObjects[i]) && !inShadow){
        Object::IntersectionValues interL = sceneObjects[i]->intersect(closestValues.P, lightDir);//Intersection lumiére.
        vec4 dirShadow = interL.P - closestValues.P; //Direction
        double distShadow = sqrt(dot(dirShadow, dirShadow)); //Distance Objet proche
        //Si distance objet < distance lumiére alors omnbre
        inShadow = (distShadow < distLight);
      }
    }
    //direction de vue -> point d'intersection
    vec4 C = normalize(cameraPosition - closestValues.P);
    //direction de la lumière -> point d'intersection
    vec4 L = normalize(lightDir);
    vec4 rayDir = normalize(E); 
    // normale du point d'intersection
    vec4 normal = normalize(closestValues.N);


    vec4 rayR = 2*dot(normal,L)*(normal-L);

  /*  if(closestObject->name == "Mirrored Sphere"){
      vec4 dirMir = rayDir - 2*dot(normal, rayDir)*normal;
      vec4 rayMir = p0 - dirMir; 
      color = castRay(p0, rayMir,NULL,depth+1);
      return color;
    }
    else if(closestObject->name == "Glass sphere"){
      double n1 = closestObject->shadingValues.Kt;
      double n2 = closestObject->shadingValues.Kr;
      vec4 normal_transparent = normal;
      //On sort
      if(dot(rayDir, normal)>0){
        n1 = closestObject->shadingValues.Kr;
        n2 = closestObject->shadingValues.Kt;
        normal_transparent = -normal;
      }
      double radical = 1-sqrt(n1/n2)*(1-sqrt(dot(normal_transparent,rayDir)));
      if(radical > 0){
        vec4 dirRef = (n1/n2)*(rayDir - dot(rayDir, normal_transparent)*normal_transparent) - normal_transparent * sqrt(radical);
        vec4 rayRef = p0 - dirRef; 
        color = castRay((p0 - normal_transparent*0.001), rayRef,closestObject,depth+1);
      } 
      else{
    ...
    //formule 
      float Ka, Kd, Ks, Kn;
      Ka = closestObject->shadingValues.Ka;
      Kd = closestObject->shadingValues.Kd;
      Ks = closestObject->shadingValues.Ks;
      Kn = closestObject->shadingValues.Kn;

      color4 Ia(closestObject->shadingValues.color.x * Ka, closestObject->shadingValues.color.y * Ka,
      closestObject->shadingValues.color.z * Ka, 1.0);

      color4 Id(closestObject->shadingValues.color.x * Kd,closestObject->shadingValues.color.y * Kd,
      closestObject->shadingValues.color.z * Kd, 1.0 );

      color4 Is(closestObject->shadingValues.color.x * Ks,closestObject->shadingValues.color.y * Ks,
      closestObject->shadingValues.color.z * Ks, 1.0 );
      color4 Iap  = GLState::light_ambient * Ia;
      color4 Idp  = GLState::light_diffuse * Id * dot(L, normal) ;
      color4 Isp = GLState::light_specular * Is * pow(dot(rayR, C),Kn) ;

      color = Iap + Idp + Isp ;
     
    }
    if(inShadow) return vec4(0.0,0.0,0.0,1.0);
    else return color;
 // }
